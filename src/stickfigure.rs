...
impl Stickfigure {
    ...
    pub fn to_jsobject(&self) -> JsValue {
        serde_wasm_bindgen::to_value(&self.inner.borrow().to_serializable())
            .expect("Something went wrong with serializing Stickfigure.")
    }

    pub fn add_polyfill(&self, options: JsValue) -> Result<Polyfill, JsError> {
        let opts: PolyfillOptions = if !options.is_undefined() {
            serde_wasm_bindgen::from_value(options)
                .map_err(|e| JsError::new(&format!("Invalid polyfill options: {}", e)))?
        } else {
        ...
        let rc_poly: Rc<RefCell<sticknodes_rs::Polyfill>> = {
            let mut inner: std::cell::RefMut<'_, sticknodes_rs::Stickfigure> =
                self.inner.borrow_mut();

            let polyfill: sticknodes_rs::Polyfill =
                sticknodes_rs::Polyfill::from_options(final_options, self.inner.borrow().clone())?;
    ...
    pub fn get_polyfill(&self, anchor_draw_index: usize) -> Result<Polyfill, JsError> {
        let option = self
            .inner
            .borrow()
            .get_polyfill(DrawOrderIndex(anchor_draw_index as i32));

        match option {
            Some(polyfill) => Ok(Polyfill::new(Rc::clone(&polyfill), Rc::new(self.clone()))),
            None => Err(JsError::new(
                format!(
                    "Polyfill not found. Searched for index {}.",
                    anchor_draw_index
                )
                .as_str(),
            )),
    ...
    pub fn get_all_polyfills(&self) -> Vec<Polyfill> {
        self.inner
            ...
            .polyfills
            .iter()
            .map(|poly| Polyfill::new(Rc::clone(poly), Rc::new(self.clone())))
            .collect()
    }

    pub fn get_node(&self, draw_index: usize) -> Result<Node, JsError> {
        if let Some(rc_node) = self
            .inner
            .borrow()
            .get_node(DrawOrderIndex(draw_index as i32))
        {
            ...
            Err(JsError::new(
                format!("Node not found. Searched for index {}.", draw_index).as_str(),
            ))
        }
    }
    ...
    pub fn get_nodes(&self, draw_indices: Vec<usize>) -> Result<Vec<Node>, JsError> {
        draw_indices
            .iter()
            .map(|draw_index| self.get_node(*draw_index))
            .collect()
    }
...
impl Stickfigure {
    pub(crate) fn add_node(
        &self,
        options: sticknodes_rs::NodeOptions,
        parent_draw_index: DrawOrderIndex,
    ) -> Result<Node, JsError> {
        let draw_index = {
            self.inner.borrow_mut().add_node(
                sticknodes_rs::Node::from_options(options),
                parent_draw_index,
            )?
        };
    ...
    pub(crate) fn get_sibling_node_draw_indices(
        &self,
        draw_index: DrawOrderIndex,
    ) -> Vec<DrawOrderIndex> {
    ...
    pub(crate) fn change_draw_index(
        &self,
        draw_index: DrawOrderIndex,
        ...
        self.inner
            .borrow_mut()
            .change_draw_index(draw_index, new_draw_index)
    }
    ...
    pub fn set_anchor_node_draw_index(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        draw_index: DrawOrderIndex,
    ) -> Result<(), JsError> {
        Ok(polyfill_rc
            .borrow_mut()
            .set_anchor_node_draw_index(draw_index, self.inner.borrow().clone())?)
    }

    pub(crate) fn set_attached_node_draw_indices(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        ...
        Ok(polyfill_rc
            .borrow_mut()
            .set_attached_node_draw_indices(draw_indices, self.inner.borrow().clone())?) // cloning stickfigure to check indices should be fine, but keep an eye on it
    }

    pub(crate) fn insert_attached_node_draw_indices_after(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        draw_indices: Vec<DrawOrderIndex>,
        draw_index: DrawOrderIndex,
        ...
        Ok(polyfill_rc
            .borrow_mut()
            .insert_attached_node_draw_indices_after(
                draw_indices,
                draw_index,
                self.inner.borrow().clone(),
            )?) // cloning stickfigure to check indices should be fine, but keep an eye on it
    }

    pub(crate) fn insert_attached_node_draw_indices_before(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        draw_indices: Vec<DrawOrderIndex>,
        draw_index: DrawOrderIndex,
        ...
        Ok(polyfill_rc
            .borrow_mut()
            .insert_attached_node_draw_indices_before(
                draw_indices,
                draw_index,
                self.inner.borrow().clone(),
            )?) // cloning stickfigure to check indices should be fine, but keep an eye on it
    ...
    pub(crate) fn remove_attached_node_draw_indices(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        draw_indices: Vec<DrawOrderIndex>,
    ) -> Result<(), JsError> {
        Ok(polyfill_rc
            .borrow_mut()
            .remove_attached_node_draw_indices(draw_indices, self.inner.borrow().clone())?) // cloning stickfigure to check indices should be fine, but keep an eye on it
    }

    pub(crate) fn try_set_attached_node_draw_indices(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        draw_indices: Vec<DrawOrderIndex>,
    ) -> Vec<DrawOrderIndex> {
        polyfill_rc
            .borrow_mut()
            .try_set_attached_node_draw_indices(draw_indices, self.inner.borrow().clone()) // cloning stickfigure to check indices should be fine, but keep an eye on it
    }
    ...
    pub(crate) fn try_remove_attached_node_draw_indices(
        &self,
        polyfill_rc: Rc<RefCell<sticknodes_rs::Polyfill>>,
        draw_indices: Vec<DrawOrderIndex>,
    ) -> Vec<DrawOrderIndex> {
        polyfill_rc
            .borrow_mut()
            .try_remove_attached_node_draw_indices(draw_indices, self.inner.borrow().clone()) // cloning stickfigure to check indices should be fine, but keep an eye on it
    }
}
