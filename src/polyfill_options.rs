...
impl From<sticknodes_rs::PolyfillOptions> for PolyfillOptions {
    fn from(options: sticknodes_rs::PolyfillOptions) -> Self {
        ...
        PolyfillOptions {
            ...
            attached_node_draw_indices: Some(options.attached_node_draw_indices.iter().map(|i| i.0).collect()),
        }
    }
}
...
impl From<PolyfillOptions> for sticknodes_rs::PolyfillOptions {
    fn from(options: PolyfillOptions) -> Self {
        ...
        let anchor_node_draw_index = options.anchor_node_draw_index.unwrap_or(defaults.anchor_node_draw_index.0);
        let color = options.color.unwrap_or(Color::from(defaults.color));
        let use_polyfill_color = options.use_polyfill_color.unwrap_or(defaults.use_polyfill_color);
        let attached_node_draw_indices = options.attached_node_draw_indices.unwrap_or(defaults.attached_node_draw_indices.iter().map(|i| i.0).collect());
        ...
        sticknodes_rs::PolyfillOptions {
            ...
            color: color.into(),
            use_polyfill_color,
            attached_node_draw_indices: attached_node_draw_indices.iter().map(|i| DrawOrderIndex(*i)).collect(),
        }
    }
}